...
- (NSString *)secureRandomNumber
{
    ...
    int errorCode = 0;
    errorCode = SecRandomCopyBytes(kSecRandomDefault, [data length], data.mutableBytes);
                ...
                if (!error) {
                    if ([response respondsToSelector:@selector(statusCode)]) {
                        if ([(NSHTTPURLResponse *) response statusCode] < 300) {
                            ...
                            completion(nil, [NSError errorWithDomain:@"Payeezy Error Info" code:[(NSHTTPURLResponse *) response statusCode] userInfo:errorObject]);
                        }
                    }
                }else{
